<template>
  <div>
    ...
    <el-row :gutter="10">
      <el-col :xs="24" :sm="8" v-for="item in suites" :key="item.id" :offset="0">
        <el-card :body-style="{ padding: '10px', height: cardHeight }" shadow="hover">
          <div class="cover">
            <router-link :to="{ name: 'mzitu-suites-detail', params: { id: item.id }}">
          ...
          <div style="padding: 5px 10px; height: 130px">
            ...
            <div class="bottom clearfix">
              <el-tag size="mini" v-for="tag in item.tags" :key="tag.id">
                <router-link
                  :to="{ name: 'mzitu-suites-card', query: { tag_id: tag.id }}"
...
export default {
  ...
  methods: {
    ...
    handleSearchClick() {
      this.$emit("handleSearchClick", this.search);
    }
  }
};
</script>
